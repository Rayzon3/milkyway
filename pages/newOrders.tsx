...
const NewOrders = () => {
    ...
    useEffect(() =>{
        axios.get('http://localhost:5000/api/providerStock/getOrders',{
    ...
    const accept = (order) =>{
        ...
        axios.put('http://localhost:5000/api/providerStock/acceptOrder',{
            orderID:order[1].id
        },{
            withCredentials: true
        })
  ...
  return (
    <div>
        ...
        <h1 className='text-center mt-8 text-3xl font-bold'>These are your pending Orders:</h1>
        <div>
            {
                mix.map((order) =>{
